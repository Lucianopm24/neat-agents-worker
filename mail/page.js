...
const BASE_CSS = [
  ...
  "a{color:var(--acc)}summary{cursor:pointer;color:var(--mut)}h2{font-size:16px;margin-bottom:10px}h3{font-size:14px;margin:14px 0 8px}details{margin-top:8px}hr.b{border:none;border-top:1px solid var(--line);margin:18px 0}",
].join("\n");
...
function renderList(){var el=document.getElementById('list');if(!el)return;document.getElementById('lcount').textContent='('+msgs.length+')';
 ...
 var h='';msgs.forEach(function(m){h+='<div class="msg'+(m.is_read?'':' unread')+'" data-act="open" data-id="'+esc(m.id)+'"><div style="min-width:0"><div class="sub">'+esc(m.subject)+'</div><div class="mut mono">'+esc(m.sender)+' → '+esc(m.address)+'</div></div><div class="mut" style="white-space:nowrap;font-size:12px">'+fmtDate(m.created_at)+'</div></div>';});el.innerHTML=h;}
...
function msgsOf(addr){api('/admin/messages?address='+encodeURIComponent(addr)+'&limit=100').then(function(j){if(!j.success)return;
 ...
 j.data.messages.forEach(function(m){h+='<div class="msg'+(m.is_read?'':' unread')+'" data-act="open" data-id="'+esc(m.id)+'"><div style="min-width:0"><div class="sub">'+esc(m.subject)+'</div><div class="mut mono">'+esc(m.sender)+'</div></div><div class="mut" style="font-size:12px">'+fmtDate(m.created_at)+'</div></div>';});
...
function orphans(){api('/admin/orphans?limit=100').then(function(j){if(!j.success)return;
 ...
 j.data.orphans.forEach(function(m){h+='<div class="msg" data-act="open" data-id="'+esc(m.id)+'"><div style="min-width:0"><div class="sub">'+esc(m.subject)+'</div><div class="mut mono">'+esc(m.sender)+' → '+esc(m.address)+'</div></div><div class="mut" style="font-size:12px">'+fmtDate(m.created_at)+'</div></div>';});
...
if(!S.tok||!S.user||S.user.role!=='admin'){document.getElementById('app').innerHTML='<div class="card"><h2>Solo el admin</h2><p class="mut">Entra primero al webmail con la cuenta admin.</p><button data-act="home">Ir al webmail</button></div>';}
else{
